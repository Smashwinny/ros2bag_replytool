...
当同一个 bag 和 target YAML 已有缓存、但 ESKF 源码或编译产物指纹发生变化时，播放器
不会静默使用旧结果，也不会直接开始产生新日志。每个受影响的播放器会提示“编译产物已变化”：
确认后只删除该 bag 对应的旧缓存索引，并立即开始一次新的完整计算；取消则保留旧缓存且
不启动回放。缓存清理不删除旧的 eskf_process.jsonl，过程日志仍可从日志浮窗单独管理。
